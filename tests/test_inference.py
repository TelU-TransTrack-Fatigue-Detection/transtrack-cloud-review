...
def _mock_video():
    return patch("inference._read_video_properties", return_value=FAKE_VIDEO_INFO)


@pytest.mark.asyncio
async def test_run_inference_returns_required_keys():
    with _mock_video(), patch("inference.asyncio.sleep", new_callable=AsyncMock):
...
async def test_run_inference_always_true_mock():
    ...
    with patch("inference.run_inference", side_effect=always_true):
        from inference import run_inference as patched
...
async def test_run_inference_review_result_true_when_confidence_high():
    with _mock_video(), patch("inference.asyncio.sleep", new_callable=AsyncMock):
        with patch("inference.random.randint", return_value=95):
            result = await run_inference("/tmp/video.mp4", "eyes_closed")
    assert result["review_result"] is True
...
async def test_run_inference_review_result_false_when_confidence_low():
    with _mock_video(), patch("inference.asyncio.sleep", new_callable=AsyncMock):
        with patch("inference.random.randint", return_value=40):
            result = await run_inference("/tmp/video.mp4", "eyes_closed")
...
async def test_run_inference_graceful_on_unreadable_video():
    with patch("inference.asyncio.sleep", new_callable=AsyncMock):
